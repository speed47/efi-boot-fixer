...
the ESP is the obvious choice, which is where bootfixr looks:

    /esp/BOOTFIXR/         (SteamOS mounts the ESP at /esp)
...
# bootfixr archive format. Must match crates/gptcore/src/backup.rs.
MAGIC = b"EFIGPTBK"
...
def mbr_health(block, last_block):
    ...
    if any(i != idx and any(r) for i, r in enumerate(records)):
        return False, "HYBRID MBR -- bootfixr refuses to touch this disk"
...
def parse_archive(blob):
    ...
    if blob[:8] != MAGIC:
        raise Fatal("not an bootfixr GPT snapshot (bad magic)")
...
def snapshot(disk, path, allow_same_disk):
    ...
    if not allow_same_disk and same_disk(directory, disk.path):
        raise Fatal(
            ...
            "because bootfixr can read it back without booting an OS."
            % (directory, disk.path)
        )
